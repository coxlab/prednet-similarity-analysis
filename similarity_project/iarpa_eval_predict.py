...
#X_test = (X_test.astype(np.float32))/255 #normalize the image
...
for i in plot_idx:
    for t in range(nt):
        plt.subplot(gs[t])
        plt.imshow(X_test[i,t,:,:,0], interpolation='none', cmap='gray') #imshow input(2D) or (3D with the last dimension being 3 or greater) if 1 channel od imshow(2D)
        plt.tick_params(axis='both', which='both', bottom='off', top='off', left='off', right='off', labelbottom='off', labelleft='off')
        if t==0: plt.ylabel('Actual', fontsize=7)

        plt.subplot(gs[t + nt])
        plt.imshow(X_hat[i,t,:,:,0], interpolation='none', cmap='gray')
        plt.tick_params(axis='both', which='both', bottom='off', top='off', left='off', right='off', labelbottom='off', labelleft='off')
        if t==0: plt.ylabel('Predicted', fontsize=7)
