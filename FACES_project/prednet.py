...
class PredNet(Recurrent):
    ...
    # Arguments
        ...
        output_mode: either 'error', 'prediction', 'all', 'all_R'.
            ...
            If 'all_R' the output will be the recurrent layers output flatten in one long vector
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        assert output_mode in {'prediction', 'error', 'all', 'all_R'}, 'output_mode must be in {prediction, error, all, all_R}'
    ...
    def get_output_shape_for(self, input_shape):
        if self.output_mode == 'prediction':
            ...
            out_sz = 0
            #for i in range(0, self.nb_layers): #grab all 4 layers
            #    out_sz += (input_shape[3] / 2**i) * (input_shape[4] / 2**i) * self.R_stack_sizes[i]
            for i in range(1, self.nb_layers): #grab all 4 layers
                out_sz += (input_shape[3] / 2**i) * (input_shape[4] / 2**i) * self.R_stack_sizes[i]    
            if self.return_sequences:
                return (input_shape[0], input_shape[1], out_sz)
            else:
                return (input_shape[0], out_sz)       
    ...
    def step(self, a, states):
        ...
        if self.output_mode == 'prediction':
            ...
            #for l in range(0, self.nb_layers): #start at 1, to grab the last 3 layers
            #    layer_R = K.batch_flatten(r[l])
            #    output = layer_R if l == 0 else K.concatenate((output, layer_R), axis=-1)    
            for l in range(1, self.nb_layers): #start at 1, to grab the last 3 layers
                layer_R = K.batch_flatten(r[l])
                output = layer_R if l == 1 else K.concatenate((output, layer_R), axis=-1)    
